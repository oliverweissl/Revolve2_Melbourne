...
def get_novelty_from_population(
    population: list[ModularRobot] | list[Body], cob_heuristic: bool = False, num_bins: int = 20
) -> NDArray[np.float64]:
    """
    ...
    """
    bodies = [robot.body for robot in population] if isinstance(population[0], ModularRobot) else population
...
def _normalize_cast_int(histograms: NDArray[np.float64]) -> NDArray[np.int64]:
    ...
    for i in range(instances):
        ...
        mask[
            :min(error, histogram.size)
        ] += 1 * int(error / histogram.size)  # each histogram must sum up to _INT_CASTER. Therefore, a mask is applied.
