...
def _body_to_adjusted_coordinates(bodies: list[Body]) -> list[NDArray[np.float64]]:
    ...
    """
    crds: list[NDArray[np.float64]] = [None] * len(bodies)
    i = 0
    for body in bodies:
        ...
        elements = [np.subtract((xe, ye, ze), core_position) for xe, ye, ze in product(range(x), range(y), range(z)) if body_array[xe, ye, ze] is not None]
